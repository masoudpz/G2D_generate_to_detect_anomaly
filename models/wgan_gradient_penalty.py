...
class WGAN_GP(object):
    ...
    def train(self, train_loader):
        ...
        if self.load_weight == True:
            self.G.state_dict(torch.load("generator.pkl"))
            self.G.eval()
            self.D.state_dict(torch.load("discriminator.pkl"))
            self.D.eval()

        D_chart = []
        G_chart = []
        for g_iter in range(self.generator_iters):
            print("Epoch : ", str(g_iter))
            ...
            if (g_iter) % 2 == 0:
                ...
                #     torchvision.utils.save_image(torch.tensor(images), "logs/" + str(tag) + ".jpg")

            G_chart.append(g_cost.data.cpu().numpy())
            D_chart.append(d_loss.data.cpu().numpy())
            if self.plot == True:
                plt.figure("Generator Chart" + str(g_iter))
                plt.xlabel("Epoch")
                plt.ylabel("Loss")
                save_chart = moving_average(np.abs(np.max(G_chart) - G_chart), 10)
                plt.plot(save_chart, 'blue')
                plt.savefig('G_chart.png')
                np.save('G_chart_data.npy',save_chart)

                plt.figure("Discriminator Chart")
                plt.xlabel("Epoch")
                plt.ylabel("Loss")
                plt.plot(np.abs(D_chart) + np.max(D_chart), 'blue')
                plt.savefig('D_chart.png')
                plt.close()
        self.t_end = t.time()
